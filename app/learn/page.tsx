  ...
  return (
    <AppShell activeNav="learn">
      <section className="relative overflow-hidden px-5 pb-6 pt-7">
        ...
        <p className="text-[10px] font-semibold tracking-[0.2em] text-primary">LEARNING</p>
      ...
      <section className="px-5 pb-8">
        <div className="card-soft overflow-hidden bg-gradient-to-br from-primary-muted via-white to-white p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] font-semibold tracking-wider text-primary">YOUR PROGRESS</p>
              <p className="mt-1 text-[22px] font-bold text-foreground">{overall.percent}%</p>
              <p className="mt-1 text-[11px] text-muted">
                {overall.completed} / {overall.total} Lessons 完了 · 正答率 {overall.accuracy}%
              </p>
            </div>
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-sm">
              <svg viewBox="0 0 36 36" className="h-12 w-12 -rotate-90">
                <circle cx="18" cy="18" r="14" fill="none" stroke="#e8f5f0" strokeWidth="3" />
                <circle cx="18" cy="18" r="14" fill="none" stroke="#1b7a5a" strokeWidth="3" strokeDasharray={`${overall.percent * 0.88} 88`} strokeLinecap="round" />
              </svg>
            </div>
          </div>
      ...
      <section className="px-5 pb-8">
        <div className="flex items-end justify-between">
          <h2 className="text-[13px] font-semibold text-foreground">レベル別学習コース</h2>
          <span className="text-[11px] text-muted">全8レベル</span>
        </div>
        <div className="mt-4 space-y-4">
          {categories.map((cat) => {
            ...
            return (
              <article key={cat.slug} className="card-soft p-4">
      ...
      <section className="px-5 pb-6">
        <h2 className="text-[13px] font-semibold text-foreground">今日の1問</h2>
        <div className="card-soft mt-3 p-5">
          ...
          <Link href="/learn/hair-basic" className="mt-4 flex w-full items-center justify-center rounded-xl bg-primary-muted py-3 text-[12px] font-semibold text-primary">
            Lessonで回答する
          </Link>
        </div>
      </section>
